...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const weekDistance = (runs.reduce((sum: number, a: any) => sum + a.distance, 0) / 1000).toFixed(1);
    ...
    const avgPaceSeconds = runs.length > 0 
      ? runs.reduce((sum: number, a: any) => sum + (a.moving_time / (a.distance / 1000)), 0) / runs.length
      : 0;
    const avgPaceMin = Math.floor(avgPaceSeconds / 60);
    const avgPaceSec = Math.floor(avgPaceSeconds % 60);
    
    const longestRun = (runs.length > 0 ? Math.max(...runs.map((a: any) => a.distance)) / 1000 : 0).toFixed(1);
